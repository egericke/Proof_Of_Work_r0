...
export default function HabitTracker({ dateRange }) {
    ...
    useEffect(() => {
        async function fetchAndProcessHabits() {
            // ... (Fetching and processing logic remains the same as the previous correct version) ...
            setIsLoading(true);
            setError(null);
            setCompletionMap({});
            setRawData([]);
            ...
            try {
                ...
                // --- Calculate Overall Stats ---
                 const currentDisplayMonth = new Date().getMonth();
                 ...
                 let bestStreak = 0;
                 let tempStreak = 0;
                 let loopDate = new Date(year, 0, 1);
    ...
    const renderYearlyGrid = () => {
        ...
        const months = Array.from({ length: 12 }, (_, i) => i); // 0-11
         ...
         return months.map((monthIndex) => {
             ...
             // Adjust firstDayOfMonth to be 0=Mon, 6=Sun if needed, or handle padding correctly
             let firstDayOfMonth = monthDate.getDay(); // 0=Sun, 1=Mon
              if (firstDayOfMonth === 0) firstDayOfMonth = 7; // Treat Sunday as 7 if starting week on Monday
              firstDayOfMonth = firstDayOfMonth -1; // Adjust to 0=Mon, 6=Sun

             const dayCells = [];

             // Add placeholders for days before the 1st of the month (assuming week starts Monday)
        ...
        return (
            <div className={`bg-gray-800 bg-opacity-60 rounded-lg border border-${colorClass}-500/30 p-4 backdrop-blur-sm`}>
                <div className="flex items-center justify-between">
                    <div>
                        {/* Use title from config if available, otherwise use passed title */}
    ...
    // **Main component return (unchanged from previous correct version)**
    return (
        <div className="space-y-6">
            ...
            {/* Overall Stats Cards */}
             <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {/* Dynamically update month name in statsConfig or pass title directly */}
                {renderStatCard(`Done in ${new Date(year, new Date().getMonth()).toLocaleString('default', { month: 'long' })}`, habitStats.doneInMonth, null, 'doneInMonth')}
