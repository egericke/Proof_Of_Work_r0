...
import { fallbackWorkouts, fallbackVo2Max } from '../../utils/fallbackData';
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        setVo2MaxHistory([fallbackVo2Max]);
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <div className="bg-gray-800 bg-opacity-60 rounded-lg border border-blue-500/20 p-4 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            Array(3)
              .fill(0)
              .map((_, i) => <WorkoutCard key={i} isLoading={true} />)
          ) : (workouts || []).length > 0 ? (
